    ...
    if($_SERVER["REQUEST_METHOD"] == "POST") {
       if (isset($_GET['pageno'])) {
        ...
        if($searchby=='byname'){
		 ...
		 if($searchby=='byname'){
          $sql = "SELECT * FROM gyms WHERE name LIKE '%$qsearch%' AND isActive=1 LIMIT $offset, $no_of_records_per_page";
        }
        else if($searchby=='bycity'){
          $sql = "SELECT * FROM gyms WHERE city LIKE '%$qsearch%' AND isActive=1 LIMIT $offset, $no_of_records_per_page";
        }
		$result = mysqli_query($db,$sql);
...
<div class="jumbotron jumbotron-fluid bg-dark">
  <div class="container">
    ...
    <form action="vsearch1.php" method="post" style="color:white">
 ...
 <table class="table table-dark">
    <thead>
      <tr>
        ...
        <th onclick="sortOption('sort5')">Most review first</th>
		<th onclick="sortOption('sort6')">Less review first</th>
